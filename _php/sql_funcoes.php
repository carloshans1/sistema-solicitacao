...
<?php
  ...
  function retornojson() {
    ...
    try {
      ...
      $sql="SELECT DISTINCT YEAR(tb_solicitacao.`data`) as ANO, MONTH(tb_solicitacao.`data`) as MES, tb_status.`status`, 
        (SELECT COUNT(*)  as total FROM tb_solicitacao as tso
        LEFT JOIN tb_status as tst ON tso.`status` = tst.id_status
        WHERE YEAR(tso.`data`) = YEAR(tb_solicitacao.`data`) AND MONTH(tso.`data`) = MONTH(tb_solicitacao.`data`) 
        AND tst.`status`=tb_status.`status`) as total
      FROM tb_solicitacao
      LEFT JOIN tb_status ON tb_solicitacao.`status` = tb_status.id_status
      ORDER BY YEAR(tb_solicitacao.`data`), MONTH(tb_solicitacao.`data`), tb_status.`status`;";
      ...
      //Retrieve the rows using fetchAll.
      //return $retorno = $stmt->fetchAll();
      $retorno22 = $stmt->fetchAll();
      //var_dump($retorno22); die;
      $json['solicitacao'][] = $retorno22;
      ...
      //echo json_encode($json, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);     
        ...
        return 'Erro ao conectar com o MySQL: ' . $e->getMessage();
    }
  }
